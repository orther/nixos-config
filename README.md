...
[![Build Starter Template](https://github.com/orther/nixos-config/actions/workflows/build.yml/badge.svg)](https://github.com/orther/nixos-config/actions/workflows/build.yml)
[![Statix Lint](https://github.com/orther/nixos-config/actions/workflows/lint.yml/badge.svg)](https://github.com/orther/nixos-config/actions/workflows/lint.yml)
...
- **Nix Overlays**: [Auto-loading of Nix overlays](https://github.com/orther/nixos-config/tree/main/overlays): drop a file in a dir and it runs (great for patches!)
- **Declarative Sync**: No-fuss Syncthing: managed keys, certs, and configuration across all platforms
- **Emacs Literate Configuration**: [Large Emacs literate configuration](https://github.com/orther/nixos-config/blob/main/modules/shared/config/emacs/config.org) to explore (if that's your thing)
...
https://github.com/orther/nixos-config/assets/1292576/2168d482-6eea-4b51-adc1-2ef1291b6598

#### Instant Emacs 29 thanks to daemon mode
- **GUI**

https://github.com/orther/nixos-config/assets/1292576/66001066-2bbf-4492-bc9e-60ea1abeb987

- **Terminal**

https://github.com/orther/nixos-config/assets/1292576/d96f59ce-f540-4f14-bc61-6126a74f9f52

### NixOS

https://github.com/orther/nixos-config/assets/1292576/fa54a87f-5971-41ee-98ce-09be048018b8
...
```sh
mkdir -p nixos-config && cd nixos-config && nix flake --extra-experimental-features 'nix-command flakes' init -t github:orther/nixos-config#starter
```
...
```sh
mkdir -p nixos-config && cd nixos-config && nix flake --extra-experimental-features 'nix-command flakes' init -t github:orther/nixos-config#starter-with-secrets
```
...
```
> [!WARNING]
> On MacOS, your `.zshrc` file will be replaced with the [`zsh` configuration](https://github.com/orther/nixos-config/blob/main/templates/starter/modules/shared/home-manager.nix#L8) from this repository. So make some changes here first if you'd like.
...
> Keys must be named `id_ed25519` and `id_ed25519_agenix`.
```sh
sudo nix run --extra-experimental-features 'nix-command flakes' github:orther/nixos-config#copy-keys
```

##### Create new keys
```sh
sudo nix run --extra-experimental-features 'nix-command flakes' github:orther/nixos-config#create-keys
```

##### Check existing keys
If you're rolling your own, just check they are installed correctly.
```sh
sudo nix run --extra-experimental-features 'nix-command flakes' github:orther/nixos-config#check-keys
```
...
* You can always add secrets later.
```sh
sudo nix run --extra-experimental-features 'nix-command flakes' github:orther/nixos-config#install
```
...
* This template offers you a place to keep passwords, private keys, etc. *as part of your configuration*.
```sh
sudo nix run --extra-experimental-features 'nix-command flakes' github:orther/nixos-config#install-with-secrets
```
...
The secret is now ready to use. Here's an [example](https://github.com/orther/nixos-config/blob/3b95252bc6facd7f61c6c68ceb1935481cb6b457/nixos/secrets.nix#L28) from my configuration. In the end, this creates a symlink to a decrypted file in the Nix Store that reflects my original file.

# Live ISO
Not yet available. Coming soon.

```sh
nix run --extra-experimental-features 'nix-command flakes' github:orther/nixos-config#live
```
...
Have feedback or questions? Feel free to use the [discussion forum](https://github.com/orther/nixos-config/discussions).
...
[![Star History Chart](https://api.star-history.com/svg?repos=orther/nixos-config&type=Date)](https://star-history.com/#orther/nixos-config&Date)
