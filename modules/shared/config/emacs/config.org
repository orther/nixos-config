...
This is the main configuration, but there also exists one more init file, ~init.el~ , that bootstraps ~org-mode~ before this file is interpreted. That's defined [[https://github.com/orther/nixos-config/blob/main/shared/files.nix#L5][here]].
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Set path for darwin
  (when (system-is-mac)
    (setenv "PATH" (concat (getenv "PATH") ":/Users/orther/.nix-profile/bin:/usr/bin"))
    (setq exec-path (append '("/Users/orther/bin" "/profile/bin" "/Users/orther/.npm-packages/bin" "/Users/orther/.nix-profile/bin" "/nix/var/nix/profiles/default/bin" "/usr/local/bin" "/usr/bin") exec-path)))
#+END_SRC
